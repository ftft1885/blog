...
同样需要注意的是Erlang支持的是并发而不是并行.因此看到并发我们就 Erlang不是线程模型.也就是说Erlang也和Nodejs一样是单线程或者无线程 ,然后通过子进程来利用多核.
...
Erlang完全是从Prolog语言上改造出来的.它是一个彻底的**函数式语言**.

对于Erlang,人们有一段出名的描述: Erlang可以把简单的事变得复杂,但也能把复杂的事变得简单.

这句话意思是Erlang不适合小型程序的开发,但适合用于复杂集群这些框架的开发.

下面都是今天看着很多文章总结的.手头没有现成的书,可能有错勿怪
...
Erlang程序的后缀是`.erl`.进入erlang编辑器也是通过erl命令.
...
同时也能看出`%`是行注释.Erlang没有块注释. 对了,不得不说perl也没有块注释.在我那篇说perl有块注释,那个方法是错误的.
...
Erlang说这样是为了防止锁,难道改变变量不是原子操作?

4.列表

列表类似数组,或者就是数组,想脚本语言那样也是多类型的
...
Erlang不支持i++,但是反而把列表玩的很转,Erlang中有lists这个库,提供各种各样的方法,后面再说.

5.结构体

结构体是一个复杂数据类型,可以包含前面说的数字,原子,列表等

结构体是用大括号包起来的.常见的例子是{A, B}, {1,2}
...
即便到这儿,我依然不能看懂任何一个简单的项目.因为这些项目都用到了[rebar](https://github.com/basho/rebar)这个项目构建工具.
